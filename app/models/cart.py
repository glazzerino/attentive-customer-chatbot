...
class Cart:
    ...
    async def get_items(cls, conversation_id: str) -> List[CartItem]:
        ...
        cursor.execute(
            "SELECT * FROM cart_items WHERE conversation_id = ?",
            (conversation_id,)
        )
    ...
    @classmethod
    async def add_item(cls, conversation_id: str, product_id: str, quantity: int = 1) -> CartItem:
    ...
    @classmethod
    async def remove_item(cls, conversation_id: str, product_id: str, quantity: Optional[int] = None) -> bool:
    ...
    async def clear(cls, conversation_id: str) -> None:
        ...
        cursor.execute(
            "DELETE FROM cart_items WHERE conversation_id = ?",
            (conversation_id,)
        )
